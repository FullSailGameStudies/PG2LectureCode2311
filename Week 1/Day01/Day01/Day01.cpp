...
int main()
{
	...
	*/
	...
	/*
		CHALLENGE #1:
			...
			Call the method on the t1000 calculator instance and
			print the results.

	*/
	bool amIEven = t1000.isEven(n5);
	//ternary operator
	//(condition) ? true : false
	std::cout << "Is " << n5 << " even? " << ((amIEven) ? "TRUE" : "FALSE") << "\n";
	...
	*/
	...
	names.erase(names.begin());//removes the first item
	...
	*/
	...
	{
		grades.push_back(rand() % 101);//% remainder
	}
	std::cout << "PG2 Grades for November\n";
	for (size_t i = 0; i < grades.size(); i++)
	{
		std::cout << std::setw(15) << grades[i] << "\n";
	}
